...
# The AppDB schema list is generated by query over
# source_db dblink, so that link must be set up before
# calling this function.
function appdb_schema_list {
  local source_database="$1"
  local login="$2"
  local passwd="$3"
  local import_dblink="$4"
  local schemas="$(cat <<EOF |
    set pagesize 0
    set feedback off
    select upper(name) from core.databaseinfo@$import_dblink;
EOF
  sqlplus -S -L $login/$passwd@$source_database
  )"
  # want one-line list, so do not quote $schemas return
  echo $schemas
}
...
function appdb_excludes_list {
  local excludes='# Export excludes.'
  while read line; do
    line=${line%%#*}  # strip comment (if any)
    local excludes
    excludes="$(printf '%s\n%s' "${excludes}" "$line")" || errexit "unable to set user export excludes for parfile"

  done < "${OPR_HOME}/conf/oprdb.appdb.excludes"
  echo "${excludes}"
}
